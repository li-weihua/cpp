...
static inline __m256 exp256_ps(__m256 &x)
{
  ...
  xs = _mm256_add_ps(_mm256_mul_ps(xs, xf), p1); //8
  xs = _mm256_add_ps(_mm256_mul_ps(xs, xf), p0); //8
  xp = _mm256_sub_ps(xp, xs);
  xp = _mm256_add_ps(xp, _mm256_set1_ps(bias)); //3
  xp = _mm256_mul_ps(xp, _mm256_set1_ps(shift)); //5

  __m256i xi = _mm256_cvtps_epi32(xp); //3
  xp = _mm256_castsi256_ps(xi); 
  return _mm256_and_ps(xp, mask); //6
}
...
int main()
{
  ...
  */
  ...
  {
    r = _mm256_add_ps(r, exp256_ps(v8fx));
  }
  );
